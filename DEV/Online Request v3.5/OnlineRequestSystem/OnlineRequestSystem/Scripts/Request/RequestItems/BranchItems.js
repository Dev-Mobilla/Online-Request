$(document).on('blur', '#txtBranch', function () {
    ...
    if (lastChild == 'Cancelled') {
        ...
        $.ajax({
            type: "POST",
            url: Url + '/RequestItems/insertBranchItemStatus',
            data: { ReqNo: ReqNO, description: Desc, quantity: Quantity, isCheckedBranch: 1 },
...
$(document).on('click', '#chkBranch', function () {
    ...
    if (lastChild == 'Cancelled') {
        ...
        $.ajax({
            type: "POST",
            url: Url + '/RequestItems/insertBranchItemStatus',
            data: { ReqNo: ReqNO, description: Desc, quantity: 0, isCheckedBranch: 0 },
            success: function (result) {
                if (result.status) {
